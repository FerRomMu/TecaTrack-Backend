...
def domain_exception_handler(request: Request, exc: TecaTrackError) -> JSONResponse:
    ...
    If the exception type is present in EXCEPTION_MAP, the corresponding (status_code, message)
    pair is used; otherwise the handler returns a 500 status with the message "Internal domain error".
    
    Parameters:
        exc (TecaTrackError): Domain exception instance to convert into an HTTP response.
    
    Returns:
        JSONResponse: Response with `status_code` set to the mapped HTTP code and body `{"detail": <message>}`.
    """
...
def setup_exception_handlers(app: FastAPI):
    ...
    Adds an exception handler that converts any `TecaTrackError` raised during request handling into a JSON response with an appropriate HTTP status code and message.
    """
