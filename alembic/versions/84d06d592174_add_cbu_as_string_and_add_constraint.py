...
def upgrade() -> None:
    """
    Add a non-null 22-character `cbu` column to the `accounts` table and enforce its
    format and uniqueness.

    Adds a CHAR(22) column named `cbu` (non-nullable), creates the check constraint
    `ck_cbu_format` to require `cbu` match the regex `^[0-9]{22}$`, and creates the
    unique constraint `uq_accounts_cbu` on `cbu`.
    """
...
def downgrade() -> None:
    ...
    Drops the unique constraint `uq_accounts_cbu` and the check constraint
    `ck_cbu_format`, then removes the `cbu` column.
    """
